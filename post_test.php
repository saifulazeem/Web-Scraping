...
$qry=$con->prepare("SELECT * FROM post_records WHERE pstore_status=1 AND update_status=0 AND post_id IS NULL AND id<=10550");
$qry->execute();
...
if($result->num_rows>0)
{
    while($row=$result->fetch_assoc())
    {
        ...
        if ($post_id==NULL)
        {
            echo 'Post id is NuLL : '.$record_id;
            echo '<br>';
            echo '<br>';
...
///                
            ...
            $title_dom1 = mysqli_real_escape_string($con, $title_dom1);
            $new_post = array(
            ...
            if($new_post_id)
            {
                $title_dom1= strval($title_dom1);
                $finaltext .= '<br>Yay, I made my new custom post.<br>';
                
                $q=$con->prepare("UPDATE post_records SET post_id=?, post_title=?, updated_on=? WHERE id=?");
                $q->bind_param("ssss", $new_post_id, $title_dom1, $last_updated, $record_id);
                // $q = mysqli_query($con, 'UPDATE post_records SET post_id='.$new_post_id.', post_title="'.$title_dom1.'", updated_on="'.$last_updated.'" WHERE id='.$record_id.'');
                // $q=$con->prepare("INSERT INTO post_records(post_id) VALUES('{$new_post_id}')");

                $q->execute();
                if(!$q){
                    echo "Error ".mysqli_error($con);
                }else{
                    echo "Inserted the record ".$title_dom1;
                }
                $q->close();
        ...
        }// if End for Post_id check
        else
       {
        echo 'Post NOT NuLL  UsE UPDATER: '.$record_id;
        echo '<br>';
        echo '<br>';
       }
       ...
       echo '<h1>Post publiashed on: '.$record_id.'</h1>';
    }//End While loop


}//End if For Record Check
